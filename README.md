...
Vocabulary mastery is tracked independently for reading, listening, and recall. Recent-performance windows and question-count spacing make the review queue react to current weakness, while a focused trouble-word mode is available from the page. The top dashboard shows stage, session accuracy, reviews due, mastered words, and current choice difficulty.
